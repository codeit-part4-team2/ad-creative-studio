...
- 소상공인이 선택한 러시아워 결과를 3장면·한국어 TTS 기반 무표정 AI 코믹 쇼츠로 변환하고 사람 승인 후 내부 노출·YouTube 예약 게시
...
결과 (이미지 + 문구) → History 저장
  ↓ 러시아워 결과에서 운영자가 코믹 쇼츠 선택
대표 이미지 1장 재사용 + 기존 /infer 순차 2회 → CPU MeloTTS 4문장 → FFmpeg 9:16 렌더
  ↓
사람의 발음·자막·제품 보존 검수 → 내부 노출 승인 → 선택 시 YouTube 예약 게시
```
