...
- 동일 요청은 멱등이며, 정상 게시 중이거나 완료된 승인 이후 다른 예약 조건으로 변경하면 `409`입니다.
- `failed`, `auth_required`, `schedule_expired` 게시 실패는 유효한 새 예약 시각으로 같은 승인 API를 다시 호출하면 `pending`으로 재대기합니다.
- 재시도 가능한 HTTP 오류를 모두 소진했거나 예상하지 못한 게시 예외가 발생하면 결과가 불명확할 수 있으므로 `needs_review`로 전환합니다.
- `needs_review`는 중복 게시 위험이 있어 자동 재시도하지 않으며 YouTube 결과를 수동 확인해야 합니다.
