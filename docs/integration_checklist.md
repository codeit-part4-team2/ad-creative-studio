...
## 러시아워 쇼츠 승인 워크플로 (2026-08-08)

### 자동 및 로컬 검증 완료

- [x] 저장된 상품명·헤드라인·서브카피·소구점만 사용하는 스토리보드
- [x] 원본 이미지를 자르지 않고 1080x1920 안에 맞추는 렌더링
- [x] FFmpeg/ffprobe로 H.264/AAC 및 10~15초 길이 검증
- [x] TTS·나레이션 없이 음악과 자막만 사용
- [x] 음악 미구성 시 무음 AAC 미리보기와 `music_warning`
- [x] 렌더/승인/게시 상태 분리 및 `var/store.json` 영속화
- [x] 원본 지문과 MP4 SHA-256 승인 직전 재검증
- [x] 승인 전 내부 노출 및 YouTube 업로드 차단
- [x] KST 러시아워 구간과 최소 10분 리드타임 검증
- [x] YouTube 실패와 내부 노출 승인 상태 분리
- [x] 토큰·OAuth 경로를 노출하지 않는 상태 API
- [x] History 화면 미리보기·무음 확인·승인·거절·예약 제어

### 승인된 외부 자산이 있어야 검증 가능

- [ ] 4개 톤별 상업적 이용 가능 음악 파일과 라이선스 증빙
- [ ] `assets/music/private/manifest.json`의 실제 파일 SHA-256 일치
- [ ] 실제 음악의 loudnorm, fade-in/out, 루프 구간 청취 검수
...
실제 OAuth, 비공개 업로드, 예약 게시, 공개 전환은 자동 테스트 범위가 아니며 각각 별도 승인이 필요합니다.
